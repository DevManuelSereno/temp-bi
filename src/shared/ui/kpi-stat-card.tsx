...
type KPIStatCardProps = {
  className?: string;
  onClick?: () => void;
} & ({ kpi: KPI; loading?: false } | { kpi?: undefined; loading: true });
...
export function KPIStatCard({ kpi, loading, className, onClick }: KPIStatCardProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        {kpi.sparklineData && (isNarrow ? <TrendArrow trend={kpi.trend} /> : <Sparkline data={kpi.sparklineData} trend={kpi.trend} />)}
      </div>
      ...
      <div className="flex items-center gap-2 text-xs">
        <Tooltip title="Variacao em relacao ao periodo anterior">
          <span
            className={cn(
