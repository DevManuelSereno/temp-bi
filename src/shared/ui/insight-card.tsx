...
import { ArrowRight, Info, Lightning, TrendUp, Warning } from "@phosphor-icons/react";
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-sm font-medium text-card-foreground truncate">{insight.title}</span>
          <Tag
            bordered
            className={cn("!m-0 text-[10px] px-1.5 py-0 h-4 shrink-0 rounded-sm", config.badgeClass)}
          >
            {config.badge}
          </Tag>
        </div>
        <p className="text-xs text-muted-foreground line-clamp-2">{insight.description}</p>
        {insight.metric && <p className="mt-1 text-xs font-semibold text-card-foreground">{insight.metric}</p>}
      </div>
