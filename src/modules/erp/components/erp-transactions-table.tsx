...
export function ERPTransactionsTable({ transactions, loading, className }: ERPTransactionsTableProps) {
  if (loading) {
    return (
  ...
  return (
    <div className={cn("frame-card flex flex-col gap-4", className)}>
      <h3 className="font-serif text-base font-semibold text-card-foreground">Ultimas Movimentacoes</h3>
      ...
      <div className="flex flex-col gap-0">
        {transactions.map((tx, i) => (
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              <div
                className={cn(
                  ...
                  tx.type === "receita" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive",
                )}
              >
                {tx.type === "receita" ? (
            ...
            <span className={cn("shrink-0 text-sm font-semibold", tx.type === "receita" ? "kpi-positive" : "kpi-negative")}>
              {tx.type === "receita" ? "+" : "−"}
