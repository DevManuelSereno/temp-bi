...
export function CampanhasCampaignsTable({ campaigns, loading, className }: CampanhasCampaignsTableProps) {
  if (loading) {
    return (
  ...
  return (
    <div className={cn("frame-card flex flex-col gap-4", className)}>
      <h3 className="font-serif text-base font-semibold text-card-foreground">Campanhas</h3>
      ...
      <div className="flex flex-col gap-0">
        {campaigns.map((c, i) => (
          ...
          >
            <span className="col-span-2 md:col-span-3 text-sm font-medium text-card-foreground truncate">{c.name}</span>
            ...
            <span className="md:col-span-1">
              <span className={cn("inline-flex rounded-full px-2 py-0.5 text-xs font-medium", STATUS_BADGE[c.status])}>
                {STATUS_LABEL[c.status]}
              </span>
            </span>
            <span className="md:col-span-2 text-sm text-card-foreground text-right">{formatCurrency(c.spend)}</span>
            <span className="md:col-span-1 text-sm font-semibold text-card-foreground text-right">{c.leads}</span>
            <span className="md:col-span-2 text-sm text-muted-foreground text-right">{formatCurrency(c.cpl)}</span>
            <span
              className={cn(
