...
export function CRMOpportunitiesTable({ opportunities, loading, className }: CRMOpportunitiesTableProps) {
  if (loading) {
    return (
        ...
        <div className="flex flex-col gap-3">
          ...
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="flex items-center justify-between gap-4">
              <Skeleton.Input active size="small" style={{ width: 144 }} />
  ...
  return (
    <div className={cn("frame-card flex flex-col gap-4", className)}>
      <h3 className="font-serif text-base font-semibold text-card-foreground">Oportunidades Recentes</h3>
      ...
      <div className="hidden sm:grid sm:grid-cols-12 gap-2 text-xs font-medium uppercase tracking-wider text-muted-foreground border-b border-border/50 pb-2">
        ...
        <span className="col-span-3">Responsavel</span>
      ...
      <div className="flex flex-col gap-0">
        {opportunities.map((op, i) => (
          ...
          >
            <span className="col-span-3 text-sm font-medium text-card-foreground truncate">{op.name}</span>
            ...
            <span className="col-span-3 text-sm text-muted-foreground truncate">{op.assignee}</span>
            <span className="col-span-2 text-sm font-semibold text-card-foreground text-right">
              {op.value > 0 ? formatCurrency(op.value) : "—"}
            </span>
            <span className="col-span-2 text-xs text-muted-foreground text-right">{formatDate(op.updatedAt)}</span>
          </div>
        ))}
      </div>
    </div>
